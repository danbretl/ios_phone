...
{
	//If the url was changed in authenticationNeededForRequest:, we'll go back to the start 
	if ([self originalURL]) { 
		if (![[[self originalURL] host] isEqualToString:[[self url] host]] || ![[[self originalURL] scheme] isEqualToString:[[self url] scheme]] || [[[self originalURL] port] intValue] != [[[self url] port] intValue]) { 
			[self performSelector:@selector(main) onThread:[[self class] threadForRequest:self] withObject:nil waitUntilDone:NO]; 
			return; 
		} 
	} 
